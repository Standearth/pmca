 ...
 * @internal
 */
function extractParamValues(searchParams) {
    const params = {};
 ...
 * @internal
 */
function extractSelectiveParamValues(searchParams, schemaKeys) {
    const params = {};
...
class SearchParams {
    ...
    constructor(schema, options = {}) {
        ...
        if (validationResult && "value" in validationResult) {
            ...
            this.#schemaShape = Object.keys(value).reduce((acc, key) => {
                acc[key] = true;
                return acc;
            }, {});
    ...
    update(values) {
        ...
        if (result && "value" in result) {
            ...
            if (isInMemory) {
                ...
                const updatedParams = this.#updateParamsWithValidatedValues(searchParams, filteredValues, validatedResult, isInMemory);
                this.#inMemorySearchParams = updatedParams;
                return;
            }
            ...
            const newSearchParams = this.#updateParamsWithValidatedValues(searchParams, filteredValues, validatedResult, isInMemory);
            // Update URL
    ...
    reset(showDefaults) {
        ...
        if (useShowDefaults) {
            ...
            if (isInMemory) {
                ...
                this.#inMemorySearchParams = newSearchParams;
            }
            else {
    ...
    #getTypedValue(key) {
        // Choose the appropriate search params source based on updateURL option and building state
        const searchParams = !building && this.#options.updateURL ? page.url.searchParams : this.#inMemorySearchParams;
        const paramsObject = this.#extractParamValues(searchParams);
    ...
    #setValue(key, value) {
        ...
        if (result && "value" in result) {
            ...
            if (isInMemory) {
                ...
                // Update the in-memory store using the helper method
                const updatedParams = this.#updateParamsWithValidatedValues(searchParams, updateObj, validatedResult, isInMemory);
                this.#inMemorySearchParams = updatedParams;
                return;
            }
            // Handle the compression mode if enabled (only when updateURL is true)
            ...
            const newSearchParams = this.#updateParamsWithValidatedValues(searchParams, updateObj, validatedResult, isInMemory);
            // Use the shared navigation function
            this.#navigateWithParams(newSearchParams);
        }
    }
        ...
        if (options.showDefaults) {
            ...
            if (validationResult && "value" in validationResult) {
                ...
                if (options.compress) {
                    ...
                    searchParams.update(validationResult.value);
                }
                else {
